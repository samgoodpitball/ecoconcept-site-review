 ...
 * Два направления.
 *
 * Единственное место, где дизайн-система разрешает карточки: элементы не
 * сравниваются построчно, а служат развилкой. Секция сознательно короткая —
 * снимок, заголовок, строка чисел и ссылка. Объяснять, как работает насос,
 * здесь незачем: на это есть продуктовая страница, а лишний экран текста
 * только откладывает переход на неё.
 *
 * Снимки взяты из hero продуктовых страниц. Оба объекта сняты на светлом
 * фоне, поэтому лежат на белом целиком (object-contain), а не кадрируются:
 * на серой подложке у снимка насоса виден белый прямоугольник кадра.
 */
export function Directions() {
  const d = home.directions;
  return (
    <section className="border-b border-line bg-white">
      <div className="mx-auto max-w-6xl px-4 py-14 md:px-6 md:py-20">
        <div className="flex flex-col gap-3 md:flex-row md:items-baseline md:justify-between md:gap-10">
          ...
          <h2 className="text-[26px] font-bold leading-[1.1] tracking-[-0.02em] md:text-[32px]">{d.title}</h2>
        </div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 md:gap-6">
          {d.items.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="group grid grid-cols-1 overflow-hidden rounded-[12px] border border-line transition-colors duration-200 hover:border-eco sm:grid-cols-[42%_1fr]"
            >
              {/* Снимок слева, текст справа: вертикальная раскладка с картинкой
                  во всю ширину карточки давала на секцию полтора экрана. */}
              <div className="relative aspect-[16/10] border-b border-line bg-white sm:aspect-auto sm:border-b-0 sm:border-r">
                <Image
                  src={item.photo.src}
                  alt={item.photo.alt}
                  fill
              ...
              <div className="flex flex-col justify-center p-5 md:p-6">
                <span className="font-head text-[11px] font-bold uppercase tracking-[0.2em] text-eco-dark">
                  {item.name}
                </span>
                <h3 className="mt-3 font-head text-[19px] font-bold leading-[1.25] tracking-[-0.01em] text-graphite md:text-[21px]">
                  {item.title}
                </h3>

                {/* Числа идут строкой через точку: три отдельных пункта со
                    значками занимали втрое больше высоты при том же смысле. */}
                <p className="mt-3 text-[14.5px] leading-[1.5] text-muted md:text-[15px]">
                  {item.facts.join(" · ")}
                </p>

                <span className="mt-5 flex items-center gap-2.5 font-head text-[13.5px] font-semibold text-graphite transition-colors group-hover:text-eco-dark">
                  {item.link}
                  <Arrow />
                </span>
              </div>
