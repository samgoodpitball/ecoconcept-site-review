...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_leads_status" AS ENUM('new', 'inwork', 'done');
  CREATE TYPE "public"."enum_cases_tag" AS ENUM('heat', 'solar', 'climate');
  ...
  CREATE TABLE "leads" (
  	...
  	"bitrix_synced" boolean DEFAULT false,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  ALTER TABLE "cases" ADD CONSTRAINT "cases_photo_id_media_id_fk" FOREIGN KEY ("photo_id") REFERENCES "public"."media"("id") ON DELETE set null ON UPDATE no action;
  ...
  CREATE INDEX "cases_photo_idx" ON "cases" USING btree ("photo_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TYPE "public"."enum_cases_tag";`)
}
